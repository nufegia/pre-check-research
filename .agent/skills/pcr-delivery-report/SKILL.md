...
- `pcr.audit.json`: best source for fields such as `level`, `check`, `target`, `evidence`, `detail`, `review_steps`, `normal_explanations`, `confidence`, `false_positive_risk`, `tool_id`, `dependency_status`, `location`, and `evidence_id`.
...
4. Aggregate signal clusters.
   ...
   - Cluster by shared review meaning, not just by tool: for example digit concentration in multiple scales, unusual group similarity, provenance/version status, manuscript-data mismatch, code reproducibility gap.
...
7. Quality check before finalizing.
   - Verify all numeric counts match the source.
   - Verify every claim can be traced to a source field or table.
...
- Name clusters by issue meaning, such as "多项量表末位数字集中", "分组描述统计过度接近", "原始文件版本链需补充说明", "稿件表格与原始数据尚未对账", or "分析脚本复跑材料不足".
